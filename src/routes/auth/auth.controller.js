...
export async function signup(req, res, next) {
    try {
        ...
        return res.status(201)
            .cookie(config.tokenConfig.accessToken.cookie.name, accessToken, config.tokenConfig.accessToken.cookie.options)
            .cookie(config.tokenConfig.refreshToken.cookie.name, refreshToken, config.tokenConfig.refreshToken.cookie.options)
            .json({ message: `User registered successfuly`, user: userData, accessToken: accessToken, refreshToken: refreshToken })
...
export async function login(req, res, next) {
    try {
        ...
        if (!username || !password) {
            return res.status(422).json({ message: "Please fill all fields (username and password)" })
        }
